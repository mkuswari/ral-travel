...
class Profile extends CI_Controller
{
	...
	public function index()
	{
		...
		if ($this->session->userdata('role') == 'admin') {
			$this->load->view("backend/profile/index_view", $data);
		} else {
			$this->load->view("frontend/profile/index_view", $data);
		}
	}
	...
	public function updateProfile()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			// cek jika user melakukan update foto profile
			$uploadAvatar = $_FILES["avatar"];
			if ($uploadAvatar) {
				...
				if ($this->upload->do_upload("avatar")) {
					$user = $this->db->get_where("users", ["user_id" => $this->session->userdata("user_id")])->row_array();
			...
			$this->Profile_model->updateProfileData($profileData);
			$this->session->set_flashdata("message", '<div class="alert alert-success">Update Profile Berhasil</div>');
			redirect('profile');
		}
	}
